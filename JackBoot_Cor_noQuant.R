...
LB <- quantile(df2[[estimator]][idx], bounds1) %>% as.numeric()
UB <- quantile(df2[[estimator]][idx], bounds2) %>% as.numeric()
    
for (LeftOutSubj in subjects){
  for (i in seq(1:length(LB))){
  addr <- which(df2$subj!=LeftOutSubj &
                  as.numeric(df2[[estimator]])>=LB[i] &
                  as.numeric(df2[[estimator]])<UB[i])
