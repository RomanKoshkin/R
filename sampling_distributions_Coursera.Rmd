...
<li> 50\% of houses in Ames are smaller than 1,499.69 square feet (I THINK THIS ONE. CHECK). </li>
...
summarize(sample_n(ames, 1000), x_bar = mean(area))
```
...
<li> Sample size of 1000 (I THINK THIS ONE). </li>
</ol>
...
```{r loop}
sample_means50 <- ames %>%
                    rep_sample_n(size = 50, reps = 150000, replace = TRUE) %>%
                    summarise(x_bar = mean(area))
...
sample_means_small <- do(rep_sample_n(ames, size = 10, reps = 25, replace = TRUE), out = mean(.$area))
```
...
<ol>
<li> Each element represents a mean square footage from a simple random sample of 10 houses. (THIS ONE) </li>
...
ggplot(data = sample_means50, aes(x = x_bar)) + geom_histogram(binwidth = 1000)
...
# type your code for this Exercise here, and Run Document
ames %>%
  rep_sample_n(size=15, replace=FALSE) %>%
...
# type your code for this Exercise here, and Run Document
summarize(sample_means50, mean(x_bar))
# OR LIKE THAT:
do(sample_means50, out = mean(.$x_bar))
```
...
sample_means150 <- ames %>%
  rep_sample_n(size=150, replace=FALSE, reps = 10000) %>%
  summarize(price = mean(price))
  # do(price = mean(.$price)) # IS REALLY SLOW
mean(sample_means150$price)

ggplot(data = sample_means150, aes(x = price)) + geom_histogram(binwidth = 1000)
# OR LIKE THAT:
# hist(as.numeric(sample_means150$price))
```

6. Which of the following is false? 
<ol>
<li> The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`) (THAT's FALSE). </li>
